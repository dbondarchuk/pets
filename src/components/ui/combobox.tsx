...
export const Loader: React.FC = () => (
  <div role='status'>
    <svg
      aria-hidden='true'
      className='mr-2 h-14 w-14 animate-spin fill-slate-600 text-gray-200 dark:text-gray-600'
      viewBox='0 0 100 101'
      fill='none'
...
>(({ onClear, allowClear, open, className, children, ...props }, ref) => (
  <div className={cn('inline-flex flex-grow flex-row', className)}>
    <Button
      variant='outline'
      role='combobox'
      aria-expanded={open}
...
export const Combobox: React.FC<ComboboxProps> = (props) => {
  ...
  return (
    <Popover open={open} onOpenChange={onOpenChange}>
      ...
      <PopoverContent
        className={'w-[var(--radix-popper-anchor-width)] min-w-max p-0'}
      >
        <Command
        ...
        >
          ...
          {(!values || values.length) == 0 && (
            <div className='py-6 text-center text-sm'>
              {props.noResultsLabel || 'No results'}
            </div>
          )}
          <CommandList
